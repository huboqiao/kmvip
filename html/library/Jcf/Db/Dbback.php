 ...
 class Jcf_Db_Dbback{
	...
	private static $instance;
 	...
 	private function __construct(){
		...
		$conf = $application->getOption('resources');
		$this->host = $conf['db']['params']['host'];
		$this->user = $conf['db']['params']['username'];
		$this->pwd =  $conf['db']['params']['password'];
		$this->dbname = $conf['db']['params']['dbname'];
		$this->charset = $conf['db']['params']['charset'];
	...
	private function saveFile(){
		...
		$filename=$_SERVER['DOCUMENT_ROOT'].$this->back_dir.$this->dbname.date('YmdHis').".sql";
		try{
		...
		$this->sql='';
 		if($result===false){
 			...
 			$this->tips='文件保存成功!<br/>文件名：'.$filename.'<br/>大小：'.round($fsize/1024/1024,2).'兆.<br/>';
 	...
 	public function check(){
 		$this->back_dir=rtrim($_SERVER['DOCUMENT_ROOT'].$this->back_dir,'/').'/';
