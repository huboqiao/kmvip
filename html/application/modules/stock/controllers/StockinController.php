 ...
 * 
 */
class Stock_StockinController extends Jcf_Controller_Action
{
	
	function init(){
		parent::init();
		$this->model = new Stock_Model_Stockin();
	...
	function indexAction(){
		...
		$this->view->location='<h3 class="location"><a class="tohome" href="index.php">主菜单</a>&gt;<a href="/stock/stockin">商品入库</a></h3>';
	...
	function infoAction(){
		...
		$this->view->PageList = parent::formatPage(ceil($totalCount/$numPerPage),$pageNum > 0 ? $pageNum : 1,$totalCount);
		if ($list) {
			$this->view->location='<h3 class="location"><a class="tohome" href="/index.phpl">主菜单</a>&gt;<a href="/stock/stockin">商品入库</a>&gt;<span>入库详情</span></h3>';
		}
	}
	...
	function inputAction(){
		...
		if (empty($_REQUEST['oid'])) {
			...
			$oid = $_REQUEST['oid'];
			if($data = $this->model->input($_REQUEST)){
				
				$txt="成功操作 [入库]: 入库单号：[".$_REQUEST['stock_on']."]";
				...
				echo json_encode(array('stat'=>true,'message'=>'入库成功'));
	...
	function pageinputAction(){
		$type=array('采购入库','自主入库');
		$result = $this->model->pageinput($_REQUEST['oid']);
