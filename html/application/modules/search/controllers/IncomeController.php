 ...
 * 
 */
class Search_IncomeController extends Jcf_Controller_Action
{
	
	function init(){
		parent::init();
		$this->model = new Search_Model_Income();
	}
	...
	function indexAction(){
		...
		$order="";
		$dbMap="";
		$totalCountcz = count($this->model->queryczInfo($user['noid'], $where,$order));
		$totalCountzz = count($this->model->queryzzInfo($user['noid'], $where,$order));
		$totalCount = $totalCountcz+$totalCountzz;
		if($totalCountcz>$offset){
			if($totalCountcz>=$offset+$numPerPage){
				
				$list= $this->model->queryczInfo($user['noid'], $where,$order,$numPerPage,$offset);
			}else{
		...
		$this->view->location='<h3 class="location"><a class="tohome" href="/index.php">主菜单</a>&gt;<a href="/search/income">收入明细</a></h3>';
