# -*- coding: utf-8 -*-
'''
Created on 2015年2月5日
添加商品分类
@author: LiuXinwu
'''
from PyQt4 import QtGui
from application.lib.Commethods import *
from application.view.addgoodsclassliu import Ui_Dialog
from application.model.goods_model import GoodsModel

class GoodClassController(ControllerAction,Ui_Dialog):
    def __init__(self,parent=None):
        ControllerAction.__init__(self,parent)
        self.parent=parent
        
        self.treeItemIndex = QTreeWidgetItem(self.treeWidget)
        self.treeItemIndex.setText(0,u'顶级分类')
        self.treeItemIndex.setData(0,1,str(0))
        self.init()
        
        self.connect(self.treeWidget, SIGNAL('itemClicked(QTreeWidgetItem*, int)'), self.onClick) 
        self.connect(self.pushButton, SIGNAL("clicked()"),self.addClassSubmit)
        self.connect(self.pushButton_2, SIGNAL("clicked()"),self.winClose)
    
    #关闭窗口
    def winClose(self):
        self.close()
    #获取上级分类id   
    def onClick(self,item,column):  
        self.ClassParentid=str(item.data(0,1).toString())
        
    #提交添加分类
    def addClassSubmit(self):
        name=str(self.classname.text())
        if name=='':
            self.boxWarning(u'提示',u'请输入商品分类名！')
            return
        try:
            self.ClassParentid
        except:
            self.boxWarning(u'提示',u'请选择父级商品分类')
            return
        if self.ClassParentid=='':
            self.boxWarning(u'提示',u'请选择父级商品分类')
            return
        data={'name':name,'parentid':str(self.ClassParentid),'v_type':str(0)}
        if self.resource:
            for i in range(len(self.resource)):
                if str(self.resource[i]['name'])==str(name):
                    self.boxWarning(u'提示',u'该商品分类已存在！')
                    return
            try:
                re = self.goodsmodel.addClass(data)
                self.getClassInfo(re)
            except:
                self.boxWarning(u'提示', u'连接服务器超时，请重新登录！')
        else:
            try:
                re = self.goodsmodel.addClass(data)
                self.getClassInfo(re)
            except:
                self.boxWarning(u'提示', u'连接服务器超时，请重新登录！')
            
    #接收服务返回添加的状态
    def getClassInfo(self,data):
        if data['stat'] == 1:
            self.parent.treeItemIndex.takeChildren()    #删除treeItemIndex下的所有节点
            self.parent.initClass()
            button=QMessageBox.question(self,"Question",  
                                    data['msg'],  
                                    QMessageBox.Ok|QMessageBox.Cancel,  
                                    QMessageBox.Ok)  
            
            if button==QMessageBox.Ok:
                self.treeItemIndex.takeChildren()  #删除treeItemIndex下的所有节点
                self.init()
                self.classname.setText('')
                self.classname.setFocus()
                self.ClassParentid=''
            else:
                self.close()
        else:
            self.boxWarning(u'提示',data['msg'])
            
            
    
    #向服务器发出获取所有的商品分类
    def init(self):
        self.goodsmodel = GoodsModel()
        try:
            re = self.goodsmodel.getAllClass()
            if re['stat'] ==1:
                self.getGoodsClassAll(re['data'])
        except:
            self.boxWarning(u'提示', u'连接服务器超时，请重新登录！')
    #接收服务器返回来的所有商品分类
    def getGoodsClassAll(self,data):
        if data:
            self.resource=data
            self.ShowResource()  
            self.treeWidget.expandAll()
        else:
            self.resource=''
            
    #显示所有资源
    def ShowResource(self):
        for i in range(len(self.resource)):
            if str(self.resource[i]['v_type'])=='0':
                if str(self.resource[i]['parentid'])=='0':
                    self.treeItem='treeItem'+str(i)
                    self.treeItem= QTreeWidgetItem(self.treeItemIndex)
                    self.treeItem.setText(0,self.tr(str(self.resource[i]['name'])))
                    self.treeItem.setData(0,1,str(self.resource[i]['id']))
                    self.getChild(self.treeItem,self.resource[i]['id'])
                
    
    def getChild(self,dqtreeItem,id):
        for i in range(len(self.resource)):
            if str(self.resource[i]['v_type'])=='0':
                if self.resource[i]['parentid']==id:
                    self.treeItem='treeItem'+str(id)+'_'+str(i)
                    self.treeItem= QTreeWidgetItem(dqtreeItem)
                    self.treeItem.setText(0,self.tr(str(self.resource[i]['name'])))
                    self.treeItem.setData(0,1,str(self.resource[i]['id']))
                    self.getChild(self.treeItem,self.resource[i]['id'])