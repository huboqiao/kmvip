...
class GuaLinkCardController(ControllerAction,Ui_Dialog):
    ...
    def tiJiao(self):
        if self.validate():
            ...
            if cardExists['stat']:
                #从服务器获取要挂失卡的密码
                password = cardExists['data']['password']
                if pwd.hexdigest()==password:
                    #更新数据库
                    re = self.model.guashiCardForCustomer({'cnoid':str(noid)})
