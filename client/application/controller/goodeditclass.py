...
from application.view.altergoodsclassliu import Ui_Dialog
...
class GoodClassEditController(ControllerAction,Ui_Dialog):
    ...
    def editClassSubmit(self):
        ...
        if self.ClassParentid=='':
            self.boxWarning(u'提示',u'请选择父级商品分类')
            return
        dataa={'name':name,'parentid':str(self.ClassParentid)}
        for i in range(len(self.resource)):
            if str(self.resource[i]['id'])!=str(self.parent.ClassParentid):
        ...
        try:
            data={}
            data['data'] = dataa
            data['cid'] = str(self.parent.ClassParentid)
