...
from application.view.goodssalemingxi import Ui_Dialog
from application.lib.commodel import getDataThread
from application.controller.customerAccountDetail import CustomerAccountDetail


class GoodsScaleAccountDetail(ControllerAction,Ui_Dialog,PrintAction):
    
    def __init__(self,data,parent=None):
        #初始化界面
        ControllerAction.__init__(self,parent)
        #初始化打印
        PrintAction.__init__(self, u"商品销售明细")

        #设置查询的 开始、结束时间
        self.dateTimeEdit.setDate(data["start"])
        self.dateTimeEdit.setCalendarPopup(True)
        
        self.dateTimeEdit_2.setDate(data["end"])
        ...
        self.lineEdit.setText(data["goodsname"])
        self.lineEdit_2.setText(data["customer"])
        self.lineEdit_6.setText(data["supplier"])
        self.lineEdit_7.setText(data["goodstype"])
...
#         self.connect(self.pushButton,SIGNAL("clicked()"),self.showDetail)
    ...
    def setTableFormat(self):
        ...
        #表格列匹配数据库字段
        self.columnName = ["order_sn","pay_time","membername","suppliers_name","goods_name","cat_name","goods_price","goods_number","amount","amount"]
    ...
    def queryData(self):
        ...
        data = {'node':'logic','act_fun':'queryGoodsScaleAccountDetail','data':data}
        self.dayreportmodel = getDataThread(data,0,"queryGoodsScaleAccountDetail")
        self.connect(self.dayreportmodel,SIGNAL("queryGoodsScaleAccountDetail"),self.getData)
